...
learning () {
    ...
    START_TIME=$(date +%s)
    pipenv run python3 train_models.py --folder="$FOLDER" --cv_technique=$1 --cv_iterations=$CROSSVALIDATION_ITERATIONS --iterations=$HYPERPARAMETER_ITERATIONS --n_jobs=$PARALLEL_THREADS 2>&1 | tee "$FOLDER/Classification Model Training.log"
    END_TIME=$(date +%s)
    DURATION="$(($END_TIME-$START_TIME))"
